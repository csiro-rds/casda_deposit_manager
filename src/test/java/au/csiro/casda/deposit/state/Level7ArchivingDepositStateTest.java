...
public class Level7ArchivingDepositStateTest extends BaseArchivingDepositStateTest
{
    protected CasdaArchivingDepositState getState(String statusCommandString, String putCommandString)
    {
        ...
        return new Level7ArchivingDepositState(stateFactory, catalogue, volMap, ngasService, archiveStatusBuilder,
                archivePutBuilder, singleJobMonitorFactory);
    }

    protected String getTestFileId()
    {
        return "level7-121212-catalogues-bob.xml";
    }

    protected void checkTestAppenderForNgasError(Exception exception)
    {
        log4jTestAppender.verifyLogMessage(Level.WARN,
                "E122] [Deposit of Level 7 collection - Connection to NGAS lost] ["
                        + "Unable to contact NGAS for artefact bob.xml "
                        + "for Level 7 collection 121212 in ARCHIVING state.",
                exception);
    }
}
